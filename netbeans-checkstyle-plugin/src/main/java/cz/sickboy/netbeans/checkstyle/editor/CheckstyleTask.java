...
public class CheckstyleTask implements CancellableTask<CompilationInfo>, CancellableChecker.CancellationHook {
  ...
  private static final RequestProcessor THREAD_POOL = new RequestProcessor(
    CheckstyleTask.class.getName(), 10, true);
...
//            setAnnotations(fileObject, Collections.singletonList(
//                    new CheckstyleAnnotation(document, getPosition(document, 0),
//                        ex.getLocalizedMessage(), SeverityLevel.ERROR)));
    }
  }
  ...
  private static Position getPosition(final StyledDocument document, final int lineNumber,
    ...
    document.render(new Runnable() {
      ...
      public void run() {
        ...
        try {
          Position position = document.createPosition(
            offset + ((columnNumber == 0) ? columnNumber : columnNumber - 1));
          LOGGER.log(Level.INFO, "Offset of " + position.getOffset());
          //ref.set(document.createPosition(offset - NbDocument.findLineColumn(document, offset)));
          //ref.set(document.createPosition(offset + columnNumber));
          ref.set(position);
  ...
  private static class CollectingListener extends CheckstyleListener<CheckstyleAnnotation> {
    ...
    public CheckstyleAnnotation createResult(AuditEvent evt) {
      Position position = getPosition(document, evt.getLine(), evt.getColumn());
      if (position != null) {
